...
def userAccountsInfo(db_connection_formal, account_id_formal, fb_handle_formal, accessToken, brand_id):
...
def pagePosts(account_id_formal, db_connection_formal, fb_handle_formal, access_token_formal, name):
    post_edge = str(account_id_formal+'/feed')
    ...
    while until_time > (int(time.time())-(33*24*60*60)) and len(page_posts['data'])>0:
        ...
        for post in page_posts['data']:
            try:
                url = 'https://graph.facebook.com/v2.2/'+str(post['from']['id'])+'?fields=picture&access_token='+access_token_formal
                pics_info = urllib.urlopen(url)
                pic_formatted = json.loads(pics_info.read())
                try:
...
def topContributor(account_id_formal, db_connection_formal, fb_handle_formal):
    ...
    user_set = list(set(user_array)-set([account_id_formal]))
    user_list = []
