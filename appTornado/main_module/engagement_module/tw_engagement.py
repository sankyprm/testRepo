...
class FacebookEngagement(object):
    ...
    def searchPage(self,req,callback):
        ...
        try:
            ...
            pic_res = urllib.urlopen('https://graph.facebook.com/v2.2/'+screenName+'?fields=picture&access_token='+self.access_token)
            pic_res_formated =  json.loads(pic_res.read())
            res['source'] = pic_res_formated['picture']['data']['url']
            callback(json.dumps(res))
        except Exception as e:
            res = {'error' : 'NotFound'}
            callback(json.dumps(res))
    ...
    def addComp(self,req,callback):
        ...
        if id not in self.complist:
            ...
            fb_benchmark.userAccountsInfo(self.db.sociabyte, id, fb_handle, brand_id, self.fb_account_id, self.access_token)
            fb_benchmark.pagePosts(id, self.db.sociabyte, fb_handle, brand_id, self.fb_account_id, self.access_token)
        ...
        try:
            if id not in self.complist:
                ...
                if wall=='error' or foll=='error' or frnd=='error':
                    ...
                    vk_benchmark.agegroupCalculator(self.db.sociabyte,id, brand_id,self.vk_user_id)
