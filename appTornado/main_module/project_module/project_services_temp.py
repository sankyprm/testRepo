...
def FeedUrlFetch(arg_formal):
    ...
    client.sociabyte.feed_link_temp.insert(entry_list)
    client.sociabyte.rss_response_temp.insert(entry_list2)
    ...
    if results_load['responseStatus']==200:
        for feedtext in results_load['responseData']['feed']['entries']:
            ...
            if any(word in text for word in allowed_words) and not any(words in text for words in not_allowed_words):# write proper condition
                ...
                feedtext['keywords'] = []
                try:
                    feedtext['lang'] =detect(feedtext['content'].encode('utf-8'))
                except:
                    feedtext['lang'] ="en"
        ...
        if no_of_entry > 0:
            print 'dhukeche'
            client.sociabyte.rss_response_temp.insert(feedtext_list)
        client.sociabyte.feed_link_temp.update({'_id':ObjectId(idd)}, {'$set':{'fetched':'yes'}})
    else:
        pass
def dataFeed(arg_formal):
    print "arg_formal====>>>>>", arg_formal
    keystr, project_id_formal, acc_type, categ, lang, lebel = arg_formal
    feed_link_response = client.sociabyte.feed_link_temp.find({'keyword':keystr, 'project_id':project_id_formal, 'fetched':'No'}, {'url':1})
    for url_list in feed_link_response:
        ...
        if url_list['url']!='':
            ...
            rssCrawler(link_id_no_list, 50, categ, keystr, project_id_formal, lang, lebel)
def dataFeedRss(arg_formal):
    keytosearch, project_id, account_type, categ, country_list, sources, lang, lebel = arg_formal
    if len(sources)==0:
        rss_sources_res = client.sociabyte.mainstream_sources.find({'country':{'$in':country_list}}, {'_id':0})
        for res in rss_sources_res:
            fetchRssFeeds(res['url'], res['country'], categ, keytosearch, project_id, lang, lebel)
    else:
        for res in sources:
            fetchRssFeeds(res, 'not available', categ, keytosearch, project_id, lang, lebel)
def fetchRssFeeds(link_rss, loc, category, keyStr, project_id_formal, lang, lebel):
    import feedparser
    from langdetect import detect
    info = feedparser.parse(link_rss['feed_link'])
        ...
        while(i<loop_through):
            ...
            if any(word in data['content'] for word in allowed_words) and not any(words in data['content'] for words in not_allowed_words):
                if detect(data['content'].encode('utf-8')) in lang:
                    client.sociabyte.rss_response_temp.insert(data)
def fetchTwitterFeeds(arg_formal, twitter_handle_formal):
    from langdetect import detect
    print "tw_arg_formal", arg_formal
    cat, query, lebel,languages, project_id_formal = arg_formal
    twitter_res= twitter_handle_formal.search(q = query, count = 10)
...
def fetchVkFeeds(arg_formal):
    ...
    cat, query, lebel,languages, project_id_formal= arg_formal
    import json
    vkfeed_res = urllib.urlopen("https://api.vk.com/method/newsfeed.search?v=5.21&q="+query+"&appid=4451246&count=10")
...
def doSomeCalculation(project_id_formal, db_connection_formal):
    all_response = db_connection_formal.sociabyte.rss_response_temp.find({'project_id':project_id_formal}, {'_id':0})
    final_json_response = {
        'feeds':[],
        'pychart':[],
        'wordCloud':[]
